...
export class Tab2Page {
  ...
  async getData() {
    try{
      this.storage.get('isLoggedIn').then( async val => {
        ...
        fetch('https://ionbmu.gebyar-it.xyz/bmu_native/login2.php', {
          method: 'POST',
          headers: header,
        ...
        fetch('https://ionbmu.gebyar-it.xyz/bmu_native/get_bimbingan_data.php', {
          method: 'POST',
          headers: header,
  ...
  async addData() {
    ...
    try{
      const res = await axios.post('https://ionbmu.gebyar-it.xyz/bmu_native/post_capture.php', formData);
